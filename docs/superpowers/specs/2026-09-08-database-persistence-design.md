...
Review adjudication (4.0.0): The implementation uses continuation frames for existing logical settings larger than 8 MiB so a valid self-generated backup remains restorable without truncation. Page/batch aggregation is byte-bounded, but the current drivers/decoder materialize a single logical row/field. The largest individual value can therefore exceed the frame memory limit. This preserves the original no-payload-cap contract; the operator runbook explicitly documents trusted-input and memory-headroom requirements. A future stricter logical-value limit would require a separate migration/compatibility decision.
