...
Upstream synchronization: merged origin/master e9e06e6 (PR119 memory-only debug) with a normal merge. Preserve upstream migration003 blob30a1c443a71bcff26ed8693e1223c4d9ecef091a; renumber unshipped account migration004 and Activity removal005. Captures now memory-only with exact raw values/fallback provenance. Schema2/3/4 backups discard both retired histories before insertion and preserve accounts/settings/usage/routing. Independent focused merge review found no blockers. New post-sync full suite in progress; UI and root tsc/lint/build/read-only container pass.

Post-sync acceptance: all265 root files passed in fresh Bun processes (4126 pass,26 skip,0 fail). Root/UI tsc and builds pass; lint0 errors/5 baseline warnings; network-isolated read-only container verifies memory-only raw capture and no debug/Activity SQL tables. Original PR118 CI including authenticated integration passed; merge commit CI rerun next.
